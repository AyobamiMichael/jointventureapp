...
class _VotingPageState extends State<VotingPage> {
  late Future<Map<String, dynamic>?> _pollDataFuture;
  late Timer _timer;
  ...
  void initState() {
    super.initState();
    _pollDataFuture = _fetchPollData();
    _startCountdown();
  }
    ...
    try {
      ...
      for (var groupDoc in groupQuerySnapshot.docs) {
        ...
        for (var pollDoc in pollQuerySnapshot.docs) {
          ...
          setState(() {
            _secondsRemaining = pollDuration;
          });
  ...
  int _parsePollDuration(String durationStr) {
  ...
  Future<void> _submitVote(String option) async {
    ...
    try {
      ...
      //  final hasVoted = prefs.getBool('voted') ?? false;

      //if (hasVoted) {
      //ScaffoldMessenger.of(context).showSnackBar(
      //SnackBar(content: Text('You can only vote once.')),
      //);
      // return;
      //}
      ...
      await pollCollection.doc(pollDocId).update({
        'option': FieldValue.arrayUnion([option]),
      });

      //  await prefs.setBool('voted', true);
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshot) {
        ...
        final pollName = pollData['pollName'] ?? 'Poll';
        final pollMessage = pollData['pollMessage'] ?? 'No message provided';
        final pollDuration = pollData['pollDuration'] ?? 'Unknown duration';
        final imageUrl = pollData['imageUrl'];
